...
def webm_chunks_to_pcm(
    audio_chunks_base64: list,
    sample_rate: int = 16000,
    ffmpeg_binary: Optional[str] = None,
) -> bytes:
    """
    Convert multiple base64-encoded WebM chunks to combined PCM bytes.
    Uses ffmpeg concat demuxer to properly combine WebM segments.
    
    Args:
        audio_chunks_base64: List of base64-encoded WebM chunks
        sample_rate: Target PCM sample rate
        ffmpeg_binary: Optional explicit path to ffmpeg
        
    Returns:
        bytes: Combined raw PCM (s16le) samples
    """
    if not audio_chunks_base64:
        raise AudioConversionError("No audio chunks provided")
    
    # Convert each chunk individually and concatenate PCM data.
    pcm_segments: list[bytes] = []
    conversion_errors = 0
    
    for chunk_b64 in audio_chunks_base64:
        try:
            pcm_segments.append(
                webm_base64_to_pcm(
                    chunk_b64,
    ...
    if not pcm_segments:
        raise AudioConversionError(
            "Unable to convert any audio chunks to PCM. "
            f"ffmpeg failed for {conversion_errors} chunks."
        )
    
    if conversion_errors:
        # Allow processing to continue if we converted at least one chunk
        # but note that some segments were skipped.
        pass
